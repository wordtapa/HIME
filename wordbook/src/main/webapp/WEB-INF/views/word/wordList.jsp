...
<html>
	...
	<h1> [${id}'s WORD LIST ] </h1>
		...
		<c:if test="${empty wordList}">
			...
			<div class = "writeBtn">
			<input type ="button" value ="WORD WRITE" onclick ="location.href='/word/writeWord';">
			</div>
			</c:if>
		...
		<c:if test="${not empty wordList }"> 
			...
			<table border ="1">
				...
				<tr>
					...
					<td><input type = "button" value ="수정" onclick="location.href='/word/updateWord?word_order=${word.WORD_ORDER}&currentPage=${currentPage}';"></td>
					<td><input type = "button" value ="삭제" onclick="location.href='/word/deleteWord?word_order=${word.WORD_ORDER}&currentPage=${currentPage}';"></td>
